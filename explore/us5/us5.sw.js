         ...
         if (resEvent.intercepted) return sanitizeResponseStatus(resEvent.returnValue);
            ...
            if (responseCtx.body) {
                switch(request.destination) {
                    case 'script':
                    case 'worker':
                        responseCtx.body = `if (!self.__us5 && self.importScripts) importScripts('${__us5$config.bundle}', '${__us5$config.config}', '${__us5$config.handler}');\n`;
...
self.us5ServiceWorker = us5ServiceWorker;
